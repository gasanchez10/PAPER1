...
prefix_1= "/Users/german.sanchez/Desktop/PAPER1/PROD_GANITE/BINARY_MODEL/SYNTHETIC/RESULTS/"
res1 = pd.read_csv(prefix_1+"MANUAL_RESULTS_prod_vc2.csv", delimiter=",",skiprows=0)
res1.columns=["Alpha","Loss_YCF_Y","ATE_YCF_Y","ACC_YCF_Y","Loss_YCF_MANUAL","ATE_YCF_MANUAL","ACC_YCF_MANUAL","Loss_YCF_RAND","ATE_YCF_RAND","ACC_YCF_RAND" ]
res2 = pd.read_csv(prefix_1+"GANITE_RESULTS__prod_vc2.csv", delimiter=",",skiprows=0)
res2.columns=["Alpha", "Alpha_Ganite"	, "Loss_YCF_Ganite", 	"ATE_YCF_Ganite",	"ACC_YCF_Ganite"]
print(res1)
...
plt.scatter(test["Alpha_Ganite"].tolist(),test["ATE_YCF_Ganite"].tolist(), linewidth = 3,  label = 'ATE FOR A SET ALPHA GANITE')
plt.legend()
...
plt.scatter(test["Alpha_Ganite"].tolist(),test["ATE_YCF_Ganite"].tolist(), linewidth = 3,  label = 'ATE FOR INTERNAL ALPHA 1')
plt.legend()
plt.savefig("GANITE_ATE_FIXED_ALPHA"+".png")
plt.close()

test=res2[(res2["Alpha_Ganite"]>=-8)&(res2["Alpha_Ganite"]<=-8)].groupby("Alpha").mean().reset_index()
plt.xlabel('Alpha ')
plt.ylabel('ATE')
plt.title('GANITE ATE FOR FIXED ALPHA GANITE')
plt.plot(test["Alpha"].tolist(),test["ATE_YCF_Ganite"].tolist(), linewidth = 3,  label = 'ATE FOR GANITE ALPHA 1')
plt.legend()
plt.savefig("GANITE_ATE_FIXED_2"+".png")
plt.close()